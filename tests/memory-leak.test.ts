...
describe('@adtonos/logging :: Memory leak', function () {
  it('should not leak memory when logging a lot', function (done) {
    ...
    const totalMemBefore = Math.round((process.memoryUsage().rss / 1024 / 1024) * 100) / 100; // MB
    ...
    setTimeout(() => {
      const totalMemAfter = Math.round((process.memoryUsage().rss / 1024 / 1024) * 100) / 100; // MB

      assert.isBelow(totalMemAfter, totalMemBefore * 1.2, "shouldn't take more than 20% extra mem");
      done();
    }, 100);
  });
});
